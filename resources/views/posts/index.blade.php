...
@section('content')
    ...
    <div class="mt-4 mb-4">
        <a href="{{ route('post.index')}}" class="btn {{
            isset($_GET['category']) ? 'btn-outline-primary' : 'btn-primary'
        }}"> Все посты</a>
        @if (count($categories) > 0)
            @foreach($categories as $category)
                <a href=" {{ route('post.index', ['category' => $category->id]) }}" class="btn {{
                    isset($_GET['category']) && $_GET['category'] == $category->id ? 'btn-primary' : 'btn-outline-primary'
                }}">
                    {{ $category->title}}
